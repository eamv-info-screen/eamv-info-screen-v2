!(function (root, factory) {
    ...
    SimpleMarquee.prototype._setup = function () {
        ...
        this._wrappers.css({"margin-left": window.innerWidth});
        ...
        if (this._horizontal) {
            ...
            // !!!!! Custom changes !!!!!
            var wrapperWidth, lastWrapper;
            if (this._wrappers.eq(0).outerWidth() < window.innerWidth) {
                wrapperWidth = window.innerWidth + this._wrappers.eq(0).outerWidth();
            } else {
                wrapperWidth = this._wrappers.eq(0).outerWidth();
                wrapperWidth += window.innerWidth;

            }
            this._size = wrapperWidth + this._options.space;
            this._wrappers.eq(0).width(wrapperWidth);
            this._wrappers.eq(1).width(wrapperWidth);
            // !!!!!
        } else {
